...
if($_GET['erreur'] != '00000') {
	...
	$num = $db->numRows($db->query("SELECT rty_id FROM t_recharge_rec WHERE rec_trace='%s'", array($ref)));
	if ($num > 0) {
	...
	$db->query(("INSERT INTO t_recharge_rec (rty_id, usr_id_buyer, usr_id_operator, poi_id, rec_date, rec_credit, rec_trace) VALUES ('%u', '%u', '%u', '%u', NOW(), '%u', '%s')"), array(3, $usrid, $usrid, 1, $amount, $ref));
} else {
